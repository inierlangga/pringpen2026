...
// Structured Data for Brand Ambassador
export const brandAmbassadorData = {
  id: "brand-ambassador",
  title: "Brand Ambassador",
  subName: "Duta Budaya Nusantara",
  badge: "Duta Budaya",
  target: "Individu / Pelajar (SMA/SMK/sederajat), Mahasiswa & Umum Nasional (Maks. 25 Tahun)",
  targetShort: "Pelajar, Mahasiswa & Umum",
  icon: "assets/icons/logo.PNG",
  description: "Wadah representasi pemuda inspiratif dalam menggaungkan keindahan seni, pesona tradisi, dan kekayaan budaya Nusantara di panggung nasional.",
  regLink: "https://staner.id/BrandAmbassadorPringPen",
  guidebookLink: "https://drive.google.com/drive/folders/1gGl7oRyYFKx9exiUc502pniGK24yx3Vx?usp=sharing",
  subthemesTitle: "✦ Fokus Peran & Benefit",
  rulesTitle: "✦ Ketentuan Teknis & Alur Seleksi",
  subthemes: [
    "<strong>Duta & Advokasi Budaya:</strong> Menjadi representasi generasi muda dalam merawat, mempromosikan, dan menyuarakan warisan seni tradisi Nusantara di ranah publik.",
    "<strong>Digital Content Creation:</strong> Mengasah kreativitas konten digital edukatif dan persuasif melalui penugasan (task) berkala di media sosial.",
    "<strong>Public Speaking & Relasi:</strong> Melatih keterampilan artikulasi komunikasi publik serta jejaring nasional sesama pemuda pencinta kebudayaan.",
    "<strong>Hadiah & Apresiasi Bergengsi:</strong> Uang Pembinaan Juara 1–3, E-Sertifikat Nasional, eksposur portofolio resmi, dan undangan kehormatan di Acara Puncak Lentera Nusa."
  ],
  techRules: [
    "<strong>Kualifikasi Peserta:</strong> WNI perorangan (Pelajar SMA/SMK/sederajat, Mahasiswa, atau Umum) berusia maksimal 25 tahun dengan akun media sosial aktif.",
    "<strong>Alur Seleksi 4 Tahap:</strong> (1) Pendaftaran & Berkas Daring (2 Sep – 1 Des 2026), (2) Pengerjaan & Pengumpulan Task Kampanye, (3) Penjurian Independen Ahli Seni & Budaya, (4) Awarding Acara Puncak (13 Des 2026 di Gedung G PKN STAN).",
    "<strong>Bobot Penilaian:</strong> Wawasan & Relevansi Tema (30%), Public Speaking & Artikulasi (25%), Kreativitas Konten Digital (20%), Kualitas Eksekusi Task (15%), Engagement & Impact Media Sosial (10%).",
    "<strong>Penugasan (Task):</strong> Peserta mengunggah karya konten kampanye digital sesuai instruksi panitia dengan tagar resmi serta menautkan akun @sabdanusastan & @teateralir."
  ],
  contactPersons: [
    { name: "Kak Thira", phone: "0852-5413-1680", waNumber: "6285254131680" },
    { name: "Kak Naomi", phone: "0813-7396-1916", waNumber: "6281373961916" }
  ]
};
...
function initCompModal() {
  ...
  const subthemesWrap = document.getElementById('comp-modal-subthemes-wrap');
  const subthemesList = document.getElementById('comp-modal-subthemes');
  const rulesWrap = document.getElementById('comp-modal-rules-wrap');
  ...
  function openCategoryModal(catId) {
    const cat = catId === 'brand-ambassador'
      ? brandAmbassadorData
    ...
    const subthemesTitle = subthemesWrap ? subthemesWrap.querySelector('.comp-section-title') : null;
    if (subthemesTitle) {
      subthemesTitle.textContent = cat.subthemesTitle || '✦ Subtema Pilihan';
    }

    const rulesTitle = rulesWrap ? rulesWrap.querySelector('.comp-section-title') : null;
    if (rulesTitle) {
      rulesTitle.textContent = cat.rulesTitle || '✦ Ketentuan Teknis';
    }
    ...
    if (contactsContainer) {
      ...
      cat.contactPersons.forEach(cp => {
        ...
        const topic = cat.id === 'brand-ambassador'
          ? 'Pemilihan Brand Ambassador'
          : `Lomba ${cat.title} (${cat.subName})`;
        const waMsg = encodeURIComponent(`Halo Kak ${cp.name.replace(/^Kak\s+/i, '')}, saya ingin bertanya mengenai ${topic} PRINGPEN 2026`);
        cpCard.innerHTML = `
          <div class="comp-cp-info">
            <span class="comp-cp-name">${cp.name}</span>
            <span class="comp-cp-phone">${cp.phone}</span>
          </div>
          <a href="https://wa.me/${cp.waNumber}?text=${waMsg}" target="_blank" rel="noopener noreferrer" class="comp-cp-wa-btn" aria-label="Chat WhatsApp ${cp.name}">
